...
def _ner_predict(args, model, dataset):
    model.eval()
    dataset = dataset.remove_columns(["labels", "text", "tokens"])
    ...
    for batch in tqdm(data_loader, desc='Evaluating on pool data'):
        with torch.no_grad():
            inputs = {'input_ids': batch['input_ids'].to(model.device),
                      'attention_mask': batch['attention_mask'].to(model.device)}
            outputs = model(**inputs)
            logits = outputs.logits  # Assuming 'logits' is the correct attribute

            # Filter and transfer in bulk after the loop
            for i in range(logits.size(0)):
                active_logits = logits[i][inputs['attention_mask'][i] == 1]
                pred_logits.append(active_logits)

    # Transfer all logits to CPU in one go if possible
    pred_logits = [logit.cpu() for logit in pred_logits]
